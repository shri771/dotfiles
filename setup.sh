...
read -p "Do you want to create symlinks for your dotfiles? (y/N): " -n 1 -r
echo
if [[ $REPLY =~ ^[Yy]$ ]]
then
...
then
    ...
    mkdir -p "$DOTFILES_DIR/tmux/plugins/tmuxifier/layouts/"
    ln -sf "$DOTFILES_DIR/scripts/tmuxifier/"* "$DOTFILES_DIR/tmux/plugins/tmuxifier/layouts/"
else
