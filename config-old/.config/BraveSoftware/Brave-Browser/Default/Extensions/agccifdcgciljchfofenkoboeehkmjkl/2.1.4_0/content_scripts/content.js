...
const CreatePriceElement = async (Type, Color, Name, ExtraObj) => {
    switch (Type) {
        case "priceElement":
        ...
        case "TrackerButton":
            ...
            Tracker.className = CONSTANTS.RemoveElementsClassName
            Tracker.setAttribute("data-price", JSON.stringify(ExtraObj.Data))
            Tracker.setAttribute("data-code", ExtraObj.Code)
            Tracker.setAttribute("data-store", ExtraObj.Store)
            Tracker.id = "TrackerInitiator";
            Tracker.style = 'margin-left: 0.25rem;display:inline-block;background-color: ' + Color.BG + ';color: ' + Color.Color + ';padding: 0px 10px;border-radius: 0.25rem;text-decoration: none;box-shadow: rgba(0, 0, 0, 0.2) 2px 2px 5px;font-weight: 500;margin-bottom: .35rem;cursor: pointer;';
            Tracker.addEventListener("click", AddToTrackerInitiator);

            if (await IsProductTracking(ExtraObj.Code)) {
                Tracker.innerHTML = `
                    <svg height="28" viewBox="0 0 21 21" width="28" xmlns="http://www.w3.org/2000/svg" style="margin: 0px; padding: 0px; position: relative; top: .2rem;"><g fill="none" fill-rule="evenodd" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" transform="translate(3 3)"><path d="m14 1c.8284271.82842712.8284271 2.17157288 0 3l-9.5 9.5-4 1 1-3.9436508 9.5038371-9.55252193c.7829896-.78700064 2.0312313-.82943964 2.864366-.12506788z"/><path d="m12.5 3.5 1 1"/></g></svg>`;
                Tracker.setAttribute("title", "Edit Tracker")


                Return.push(Tracker);

                let Remove = document.createElement("div");
                Remove.className = CONSTANTS.RemoveElementsClassName
                Remove.setAttribute("data-price", JSON.stringify(ExtraObj.Data))
                Remove.setAttribute("data-code", ExtraObj.Code)
                Remove.setAttribute("data-store", ExtraObj.Store)
                Remove.id = "TrackerRemover";
                Remove.style = 'margin-left: 0.25rem;display:inline-block;background-color: ' + Color.BG + ';color: ' + Color.Color + ';padding: 0px 10px;border-radius: 0.25rem;text-decoration: none;box-shadow: rgba(0, 0, 0, 0.2) 2px 2px 5px;font-weight: 500;margin-bottom: .35rem;cursor: pointer;';
                Remove.innerHTML = `<svg height="28" viewBox="0 0 21 21" width="28" xmlns="http://www.w3.org/2000/svg" style="margin: 0px; padding: 0px; position: relative; top: .2rem;"><g fill="none" fill-rule="evenodd" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" transform="translate(4 4)"><path d="m12.5 10.5v-8c0-1.1045695-.8954305-2-2-2h-8c-1.1045695 0-2 .8954305-2 2v8c0 1.1045695.8954305 2 2 2h8c1.1045695 0 2-.8954305 2-2z"/><path d="m6.5 3.5v6" transform="matrix(0 1 -1 0 13 0)"/></g></svg>`;
                Remove.setAttribute("title", "Remove Tracker");
                Remove.addEventListener("click", RemoveTracker);
                Return.push(Remove);

            } else {
                Tracker.innerHTML = `<svg height="28" viewBox="0 0 21 21" width="28" xmlns="http://www.w3.org/2000/svg" style="margin: 0px; padding: 0px; position: relative; top: .2rem;"><g fill="none" fill-rule="evenodd" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" transform="translate(4 4)"><path d="m12.5 10.5v-8c0-1.1045695-.8954305-2-2-2h-8c-1.1045695 0-2 .8954305-2 2v8c0 1.1045695.8954305 2 2 2h8c1.1045695 0 2-.8954305 2-2z"></path><path d="m6.5 3.5v6.056"></path><path d="m6.5 3.5v6" transform="matrix(0 1 -1 0 13 0)"></path></g></svg><span style="position: relative; top: -.4rem;"> Set Alert</span>`;

                Tracker.setAttribute("title", "Add to Tracking");
                Return.push(Tracker);
            }
            ...
            List.className = CONSTANTS.RemoveElementsClassName
            List.style = 'margin-left: 0.25rem;display:inline-block;background-color: ' + Color.BG + ';color: ' + Color.Color + ';padding: 0px 10px;border-radius: 0.25rem;text-decoration: none;box-shadow: rgba(0, 0, 0, 0.2) 2px 2px 5px;font-weight: 500;margin-bottom: .35rem;cursor: pointer;';
            List.innerHTML = `<svg height="28" viewBox="0 0 21 21" width="28" xmlns="http://www.w3.org/2000/svg" style="margin: 0px; padding: 0px; position: relative; top: .2rem;"><g fill="none" fill-rule="evenodd" transform="translate(4 5)"><g stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="m11.5 5.5h-7"/><path d="m11.5 9.5h-7"/><path d="m11.5 1.5h-7"/></g><path d="m1.49884033 2.5c.5 0 1-.5 1-1s-.5-1-1-1-.99884033.5-.99884033 1 .49884033 1 .99884033 1zm0 4c.5 0 1-.5 1-1s-.5-1-1-1-.99884033.5-.99884033 1 .49884033 1 .99884033 1zm0 4c.5 0 1-.5 1-1s-.5-1-1-1-.99884033.5-.99884033 1 .49884033 1 .99884033 1z" fill="currentColor"/></g></svg>`;
            List.addEventListener("click", () => {
                let Url = chrome.runtime.getURL("options/settings.html") + "?#ph-tabs-tracking";

                chrome.runtime.sendMessage(
                    { action: 'OpenUrl', url: Url }, (response) => { });
            });
            List.setAttribute("title", "Check All Trackings");
            Return.push(List);
...
const UpdatePrices = async (Prices, ele) => {
    ...
    if (Prices.highest || Prices.lowest || (Prices.total && Prices.count) || Prices.lowestoffer) {
        ...
        if (Prices.highest) {
            PriceContainer.appendChild(await CreatePriceElement("priceElement", { Color: "#ff534b" }, "Highest", { Price: parseInt(Prices.highest), Display: "inline-block", InLine: false }))
        }
        if (Prices.total && Prices.count) {
            PriceContainer.appendChild(await CreatePriceElement("priceElement", { Color: "#555555" }, "Average", { Price: parseInt(Prices.total / Prices.count), Display: "inline-block", InLine: false }))
        }
        if (Prices.lowest) {
            Prices.lowest = parseFloat(Prices.lowest)
            PriceContainer.appendChild(await CreatePriceElement("priceElement", { Color: "#17a2b8" }, "Lowest", { Price: parseInt(Prices.lowest), Display: "inline-block", InLine: false }))
        }
        if (Prices.lowestoffer) {
            ...
            if (Prices.lowest) {
                ...
                if (Prices.lowest > Prices.lowestoffer) {
                    PriceContainer.appendChild(await CreatePriceElement("priceElement", { Color: "#28a745" }, "Lowest Offer Price", { Price: parseInt(Prices.lowestoffer), Display: "block", InLine: true }))
                }
            } else {
                PriceContainer.appendChild(await CreatePriceElement("priceElement", { Color: "#28a745" }, "Lowest Offer Price", { Price: parseInt(Prices.lowestoffer), Display: "block", InLine: true }))
    ...
    if ("code" in Prices && Prices.code.length > 0) {
        ...
        if (!(ExtensionConfigs && "enable-floating-button" in ExtensionConfigs && ExtensionConfigs["enable-floating-button"] === false)) {
            let ChartButton = document.createElement("div");
            ChartButton.className = CONSTANTS.RemoveElementsClassName
            ChartButton.style = "position: fixed; z-index: 1001; text-align: center; background-color: #021542; padding: 0rem 0.5rem 0.2rem 0.5rem; border-radius: 0.5rem; cursor: pointer;" + CONSTANTS.ExtensionPosition((ExtensionConfigs && "floating-button-location" in ExtensionConfigs && ExtensionConfigs["floating-button-location"].length > 0) ? ExtensionConfigs["floating-button-location"] : "");

            ChartButton.setAttribute("ChartUrl", CONSTANTS.ChartPreURL + Prices.code)

            // console.log("CurrentChartUrl", CurrentChartUrl)
            ...
            try {
                chrome.runtime.sendMessage(
                    ...
                    async (response) => {
                        if (response.status) {
                            removeElements()
                            Obj.PriceContainer = await UpdatePrices(response.data, Obj.GetElementAfterEnd());
                            Obj.AddToTracking = await CreatePriceElement("TrackerButton", { BG: "#021542", Color: "#ff534b" }, "", { Code: GetProductData.Code, Data: response.data, Store: Obj.Constants.Domain });
                            Obj.AddToTracking.forEach((element) => { Obj.GetElementAfterEnd().insertAdjacentElement("afterend", element) });
                            // Obj.PriceContainer.insertAdjacentElement('beforebegin', Obj.AddToTracking);
